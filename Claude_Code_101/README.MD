...
1. **/context** — visualizes your current context window usage as a colored grid. Shows a colored grid of context usage + optimization suggestions.
2. **/compact** — compacts everything up to that point. Handy when you want to free up context space while keeping a memory of what you previously worked on.
3. **/clear** — if you want to completely start from scratch with no memory of the previous session. This removes everything.
4. **/btw** — ask a quick side question without interrupting Claude's current work.
5. **/init** — creates a CLAUDE.md file with codebase documentation, project structure, build/test commands, and other key details for future Claude Code sessions.
...
7. **/mcp** — manage MCP (Model Context Protocol) servers — external integrations that extend Claude's capabilities with additional tools and data sources.
8. **/plugins** — Opens the plugin manager: browse, install, enable/disable, and remove plugins, and add marketplaces (repositories of plugins). Used to hook up custom    
   slash commands, agents, hooks, and MCP servers.
9. Claude --debug  — Runs Claude Code with verbose debug logging. Surfaces internal details like tool calls, MCP server activity, API requests, and errors. Useful for  
...
- **Commit.** Once you've tested the changes yourself and are happy with the results, it's time to push your code. Before you commit, run a subagent code reviewer to look at your work. A subagent gets a fresh pair of eyes on the codebase — it doesn't carry the bias the main agent might have from the session.
